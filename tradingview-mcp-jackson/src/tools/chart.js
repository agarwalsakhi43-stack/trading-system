...
export function registerChartTools(server) {
  ...
  server.tool(
    'chart_safe_set_symbol',
    'Safely switch chart symbol using the TradingView search box. ' +
    'Checks TradingView is healthy first, types into the search box (never reloads the page), ' +
    'waits 3 s, verifies the chart title matches, and retries once on failure. ' +
    'Prefer this over chart_set_symbol for batch scanning to avoid freezes.',
    { symbol: z.string().describe('Full symbol with exchange prefix, e.g. NSE:TCS, NSE:INFY, NASDAQ:AAPL') },
    async ({ symbol }) => {
      try { return jsonResult(await core.safeSetSymbol({ symbol })); }
      catch (err) { return jsonResult({ success: false, error: err.message }, true); }
    }
  );
  ...
  server.tool(
    'chart_switch_and_draw_sr',
    'DEFAULT tool for switching to a new stock. ' +
    'Switches symbol via the search box (safe, no reload), immediately clears all old drawings, ' +
    'waits 2 s for the chart to stabilize, then auto-calculates pivot-based S&R from the last 100 bars ' +
    'and draws up to 3 support (green) + 3 resistance (red) horizontal lines. ' +
    'Use this instead of chart_safe_set_symbol whenever you want a clean chart on the new stock.',
    { symbol: z.string().describe('Full symbol with exchange prefix, e.g. NSE:TCS, NSE:INFY') },
    async ({ symbol }) => {
      try { return jsonResult(await core.switchWithFreshSR({ symbol })); }
      catch (err) { return jsonResult({ success: false, error: err.message }, true); }
    }
  );
}
